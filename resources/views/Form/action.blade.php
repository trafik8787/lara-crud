@if($configNode->getButtonEdit())
    <a href="{{ url()->current()}}/{{$id}}/edit" class="btn btn-warning btn-flat "><i class="fa fa-fw fa-edit"></i> Edit</a>
@endif
...
@if($configNode->getButtonDelete())
    {!! Form::open(array('style' => 'display: inline-block;', 'class' => '', 'url' => url()->current().'/'.$id.'/delete', 'method' => 'DELETE', 'files' => false)) !!}
        {{ Form::hidden('id', $id) }}
        <button type="submit" class="btn btn-danger btn-flat"><i class="fa fa-fw fa-remove"></i> Delete</button>
    {!! Form::close() !!}
